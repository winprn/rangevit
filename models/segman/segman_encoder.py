...
from .csm_triton_encoder import CrossScanTriton, CrossMergeTriton
...
def stem(in_chans=3, embed_dim=96):
    return nn.Sequential(
            nn.Conv2d(in_chans, embed_dim//2, kernel_size=3, stride=(1, 2), padding=1, bias=False),
            nn.BatchNorm2d(embed_dim//2),
            nn.GELU(),
            nn.Conv2d(embed_dim//2, embed_dim//2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(embed_dim//2),
            nn.GELU(),
            nn.Conv2d(embed_dim//2, embed_dim, kernel_size=3, stride=(1, 2), padding=1, bias=False),
            nn.BatchNorm2d(embed_dim),
            nn.GELU(),
...
class SegMANEncoder(nn.Module):
    def __init__(self,
        ...
        image_size = [(image_size[0], image_size[1]//2**(i+2)) for i in range(4)]
        ...
        for i_layer in range(self.num_layers):
            ...
            downsample = nn.Sequential(
                nn.Conv2d(embed_dims[i_layer], embed_dims[i_layer+1], kernel_size=3, stride=(1, 2), padding=1, bias=False),
    ...
    def init_weights(self, pretrained=None):
        if isinstance(self.pretrained, str):
            ...
            print(f"loaded pretrained weights using {state_dict_name} from {self.pretrained}: {msg}")
