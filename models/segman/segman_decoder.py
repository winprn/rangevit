...
from .csm_triton_decoder import CrossScanTriton, CrossMergeTriton
...
def pixel_unshuffle_width(x, scale):
    b, c, h, w = x.shape
    if w % scale != 0:
        raise ValueError(f"Width {w} not divisible by scale {scale}")
    x = x.view(b, c, h, w // scale, scale)
    x = x.permute(0, 1, 4, 2, 3).contiguous()
    return x.view(b, c * scale, h, w // scale)
...
class ConvModule(nn.Module):
    def __init__(
        ...
        self.norm = nn.BatchNorm2d(out_channels) if norm_cfg is not None else None
        if act_cfg is False:
            self.act = None
        else:
            act_type = "ReLU" if act_cfg is None else act_cfg.get("type", "ReLU")
            if act_type.lower() == "silu":
...
class SegMANDecoder(nn.Module):
    def __init__(
        ...
        self.conv_downsample_2 = ConvModule(
                        self.embed_dim, self.embed_dim*2, kernel_size=3, stride=(1, 2), padding=1,
                        norm_cfg=dict(type='BN'))
        
        self.conv_downsample_4 = ConvModule(
                        self.embed_dim, self.embed_dim*4, kernel_size=5, stride=(1, 4), padding=2,
                        norm_cfg=dict(type='BN'))
        ...
        self.reduce_channels = nn.ModuleList([ConvModule(in_channels=self.embed_dim*4,
                                out_channels=self.embed_dim,kernel_size=1,
                            norm_cfg=dict(type='BN')) for i in range(3)])
        ...
        self.image_pool = nn.Sequential(
                                ...
                                ConvModule(self.embed_dim, self.embed_dim, 1, norm_cfg=None))
        ...
        self.cls_seg = nn.Conv2d(self.embed_dim, self.num_classes, kernel_size=1)
    ...
    def forward_winssm(self, x: torch.Tensor, c2, c3, c4, c1=None):
        ...
        x_2_unshuffle = pixel_unshuffle_width(x_2, 2)
        x_unshuffle = pixel_unshuffle_width(x, 4)
    ...
    def forward(self, inputs):
        x, c2, c3, c4 = self.forward_mlp_decoder(inputs)
