...
class RangeViT(nn.Module):
    def __init__(
        ...
        n_cls=17, # number of classes
        backbone='vit_small_patch16_384',
        image_size=(32, 384), # image size
        pretrained_path=None, # pretrained path
        
        # Patch Processing Parameters
        new_patch_size=None, # custom patch size, overide the backbone default
        new_patch_stride=None, # custom patch stride, overide the backbone default
        reuse_pos_emb=False, # whether to reuse the positional embeddings from pretrained model
        reuse_patch_emb=False, # whether to reuse the patch embeddings from pretrained model
        
        # Stem Configuration Parameters
        conv_stem='none', # whether to use a convolutional stem
        stem_base_channels=32, # Base channels for ConvStem
        stem_hidden_dim=None, # Hidden dimension in ConvStem

        # Decoder Configuration Parameters
        skip_filters=0, # Channels for skip connections (0 = no skip connections)
        decoder='up_conv', # Decoder type (linear or up_conv)
        up_conv_d_decoder=64, # Hidden dimension for up_conv decoder
        up_conv_scale_factor=(2, 8), # Scaling factor for up_conv decoder
        use_kpconv=False, # Whether to use KPConv
        ):
