...
if __name__ == '__main__':
  ...
  # populate the pointclouds
  label_a_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(label_a_paths)) for f in fn if f.endswith('.label')]
  label_a_names.sort()
  label_b_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(label_b_paths)) for f in fn if f.endswith('.label')]
  label_b_names.sort()
