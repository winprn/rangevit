...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate clean angled 3D projection inference images (correct in green, incorrect in red).")
    ...
    parser.add_argument("--yaw", type=float, default=-45.0, help="Yaw angle in degrees for 3D rotation.")
    parser.add_argument("--pitch", type=float, default=25.0, help="Pitch angle in degrees for 3D rotation.")
    parser.add_argument("--boundary_h", type=float, default=30.0, help="Horizontal scale boundary in meters.")
    parser.add_argument("--boundary_v", type=float, default=20.0, help="Vertical scale boundary in meters.")
...
def main():
    ...
    print(f"Selected random indices for angled BEV: {selected_indices}")
    
    H, W = args.bev_size, args.bev_size
    yaw_rad = np.radians(args.yaw)
    pitch_rad = np.radians(args.pitch)
    
    boundary_h = args.boundary_h
    boundary_v = args.boundary_v
    ...
    for i, idx in enumerate(selected_indices):
        ...
        # Original 3D point cloud coordinates (x, y, z)
        ...
        colors[correct_mask] = [120, 220, 120] # Soft light green for correct (BGR)
        colors[incorrect_mask] = [0, 0, 255] # Red for incorrect
        
        # Apply 3D rotation (Yaw then Pitch)
        # Yaw rotation around Z axis
        cos_y, sin_y = np.cos(yaw_rad), np.sin(yaw_rad)
        x_yaw = x * cos_y - y * sin_y
        y_yaw = x * sin_y + y * cos_y
        z_yaw = z
        
        # Pitch rotation around Y axis
        cos_p, sin_p = np.cos(pitch_rad), np.sin(pitch_rad)
        x_rot = x_yaw * cos_p + z_yaw * sin_p
        y_rot = y_yaw
        z_rot = -x_yaw * sin_p + z_yaw * cos_p
        
        # Now, project (y_rot, z_rot) onto the screen
        # Filter valid points within boundaries
        valid = (y_rot >= -boundary_h) & (y_rot <= boundary_h) & (z_rot >= -boundary_v) & (z_rot <= boundary_v)
        ...
        # Map physical coordinates (y_v, z_v) to image grid coordinates (row, col)
        cols = ((y_v + boundary_h) / (2 * boundary_h) * (W - 1)).astype(np.int32)
        rows = (((boundary_v - z_v) / (2 * boundary_v)) * (H - 1)).astype(np.int32)
        
        # Draw points (3x3 thickness)
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
        ...
        # Save image
        output_path = output_dir / f"angled_bev_frame_{frame_id}.png"
        cv2.imwrite(str(output_path), img)
        print(f"Saved angled BEV to {output_path}")
        
        if artifact_dir:
            artifact_path = artifact_dir / f"angled_bev_frame_{frame_id}.png"
            cv2.imwrite(str(artifact_path), img)
            
    print("\nAngled BEV visualizations completed successfully!")
