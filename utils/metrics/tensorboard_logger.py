...
def tensorboard_logger(epoch,
    ...
    loss_aux = loss_dict.get('loss_aux')
    loss_dice = loss_dict.get('loss_dice')
    ...
    if loss_aux is not None:
        recorder.tensorboard.add_scalar(
            tag='{}_LossAux'.format(mode), scalar_value=loss_aux.item(), global_step=epoch)
    if loss_dice is not None:
        recorder.tensorboard.add_scalar(
            tag='{}_LossDice'.format(mode), scalar_value=loss_dice.item(), global_step=epoch)
